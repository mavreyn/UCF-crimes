...
def get_embedding(text: str, openai_api_key, model='text-embedding-ada-002'):
    text = text.replace("\n", " ")
    return openai.Embedding.create(input = [text], model=model, api_key=openai_api_key)['data'][0]['embedding']
...
def search_incident_titles(df, query, embeddings_dict, openai_api_key, n=5):
    '''
    ...
    '''
    embedding = get_embedding(query, openai_api_key)
